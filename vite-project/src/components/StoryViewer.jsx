...
useEffect(() => {
  ...
  if(viewStory && viewStory.media_type !== 'video'){
    setProgress(0)
    const duration = 3000; // 3 seconds for non-video stories
    const setTime=100;
    let elapsed=0;
    setInterval(() => {
      elapsed += setTime;
  ...
  //Close story viewer after duration(10 sec)
  timer = setTimeout(() => {
    setViewStory(null)
  },duration)
}
...
[viewStory,setViewStory])
  ...
  const renderContent = () => {
    switch (viewStory.media_type) {
      ...
      case 'video':
        return (
          <video onEnded={handleClose} src={viewStory.media_url} alt=""
    ...
    return (
      <div
        className='fixed inset-0 h-screen bg-black bg-opacity-90 z-50
